...
namespace MCLawl
{
    public sealed class Player
    {
        ...
        public string ctfColor = "";
        public bool spectator = true;
        public ushort[] tntPos = new ushort[3];
        ...
        public bool hasBigTNT = false;
        public int bigTNTCount = 0;

        public int killstreak = 0;
        public int topKillstreak = 0;

        public int teamID = 0; // Red = 1, Blue = 2
        ...
        public void manualChange(ushort x, ushort y, ushort z, byte action, byte type)
        {
            ...
            if (!CTF.gameStarted && group.Permission < Server.opchatperm && !group.name.ToLower().Contains("mod"))
            {
                SendMessage("You can only build once the game has started!");
                SendBlockchange(x, y, z, b);
                return;
            }
            if (b == Block.tnt && hasTNT)
            {
                SendBlockchange(x, y, z, b);
                return;
            }
            if (b == Block.tnt && !hasTNT)
            {
                ...
                tntPos[1] = y;
                tntPos[2] = z;
                SendMessage("Place a purple block or say /t to blow up the TNT.");
            }
            if (b == Block.darkgrey && hasMine)
            {
                SendBlockchange(x, y, z, b);
                return;
            }
            if (b == Block.darkgrey && !hasMine)
            {
                ushort[] n = new ushort[3];
                n[0] = x;
                n[1] = y;
                n[2] = z;
                hasMine = true;
                SendMessage("Say /d to defuse the mine.");
                mine = new Mine(n, this, this.level, false);
                Mine.placeMine(mine, this, this.level, n);
                Thread.Sleep(5000);
                mine.active = true;
                SendMessage("Mine is now active!");
            }
            if (b == Block.purple && hasTNT && hasBigTNT)
            {
                bigTNTCount--;
                hasTNT = false;
                CTF.ExplodeTNT(this, x, y, z, 3);
            }
            if (b == Block.purple && hasTNT)
            {
                hasTNT = false;
                CTF.ExplodeTNT(this, x, y, z, 2);
                 
            }
            if (bigTNTCount == 0 && hasBigTNT)
            {
                hasBigTNT = false;
                SendMessage("Your big TNT has expired!");
        ...
        void HandleChat(byte[] message)
        {
            try
            {
                ...
                if (CTF.voting && (text == "1" || text == CTF.maps[0]))
                {
                    CTF.votes[0]++;
                ...
                if (CTF.voting && (text == "2" || text == CTF.maps[1]))
                {
                    CTF.votes[1]++;
                    SendMessage("Thanks for voting!");
                    voted = true;
        ...
        #region Capture the Flag methods

        public CTFTeam getTeam()
        {
            if (CTF.redTeam.players.Contains(this)) { return CTF.redTeam; }
            if (CTF.blueTeam.players.Contains(this)) { return CTF.blueTeam; }
            return null;

        }
        public CTFTeam opponentTeam()
        {
            if (getTeam() == CTF.redTeam) { return CTF.blueTeam; } else return CTF.redTeam;
        }

        public void takeFlag()
        {
            getTeam().takeFlag(this, getTeam());

        }
        public void dropFlag()
        {
            CTFTeam dropped = opponentTeam();
            CTF.gameLevel.SetTile(dropped.flagSpawn[0], dropped.flagSpawn[1], dropped.flagSpawn[2], dropped.blockColor);
            if (!CTF.antiStalemate) { CTF.Message(getTeam().teamColor + name + c.def + " dropped the " + dropped.teamColor + dropped.name + c.def + " flag."); }
            carryingFlag = false;
            getTeam().hasFlag = false;
            dropped.flagAtBase = true;
        }

        public void GlobalRespawn(string displayname = "")
        {
            if (displayname == "") displayname = color + name;
            foreach (Player p in Player.players)
            {
                if (p.Loading || p == this || p.level != level) { }
                else
                {
                    p.SendDie(id);
                    p.SendSpawn(id, displayname, pos[0], pos[1], pos[2], rot[0], rot[1]);
                }
            }
        }
        #endregion
